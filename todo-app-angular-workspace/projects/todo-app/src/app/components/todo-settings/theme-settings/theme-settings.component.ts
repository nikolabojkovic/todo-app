...
import { ISettings, IThemeSettings, BackgroundColor, BsThemes } from '../../../shared/models';
...
export class ThemeSettingsComponent implements OnInit {
  ...
  areas = [
    ...
    { coords: '144,75,153,79,153,90,144,94,135,90,135,79', color: '#CC99FF', position: {x: -125, y: 111} },
  ...
  // allow template to pass string literals directly
  changeBackground(background: BackgroundColor | string, bsTheme: BsThemes | string) {
    const bg = background as BackgroundColor;
    const theme = bsTheme as BsThemes;
  ...
  // simple wrapper used by template to avoid complex property access in the template
  selectArea(a: { color: string; position: { x: number; y: number } }) {
    this.changePrimaryColor(a.color, a.position.x, a.position.y);
  }
}
